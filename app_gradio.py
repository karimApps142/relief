...
def make_relief(image_path, depth_model, depth_mm, pixel_mm, depth_smooth,
                depth_compress, invert, flatten_bg, make_solid):
    if not image_path:
        raise gr.Error("Upload an image first.")
    params = ReliefParams(
        depth_model=depth_model, relief_depth_mm=depth_mm, pixel_mm=pixel_mm,
        depth_smooth=depth_smooth, depth_compress=depth_compress,
        invert=invert, flatten_bg=flatten_bg, make_solid=make_solid,
    )
...
with gr.Blocks(title="CNC Bas-Relief") as demo:
    ...
    go.click(make_relief,
             [img, depth_model, depth_mm, pixel_mm, depth_smooth,
              depth_compress, invert, flatten_bg, make_solid],
